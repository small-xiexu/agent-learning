...
很多人第一次看 `module-react-paradigm` 时，会卡在这几类问题上：

1. ReAct 里的 `while (step < maxSteps)` 到底在保护什么
2. 模型返回 `ToolCall` 之后，到底是谁去执行工具
3. 手写版 `ReActAgent` 和官方 `ReactAgent` 到底谁帮我做了什么
4. `framework-core` 里这么多接口，各自的边界在哪
5. 真实 OpenAI Demo 需要哪些配置，应该怎么跑
...
如果你把手写版和官方版拆着看，很容易只看到两堆不同代码；把它们放到同一张表里，会立刻清楚很多。
...
### 11.3 `OpenAiReactDemoPropertySupport` 为什么存在
...
## 13. ReAct、Reflection、Plan-and-Solve 的边界
