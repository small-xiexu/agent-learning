...
- 项目内教学型注释口径：详见《[代码教学注释规范](./docs/代码教学注释规范.md)》

---

## 🎯 各模式适用场景与选型建议

很多初学者第一次看这些模块时，最容易产生的误区不是“看不懂代码”，而是：

- 明明都能让 LLM 干活，它们到底差在哪
- 为什么有的问题用 ReAct 就够了，有的问题却要上 Graph Flow 或 Supervisor
- 多智能体场景里，到底该选 Conversation、CAMEL 还是 Supervisor

可以先记一个最短选型口诀：

- 缺信息、要查工具：`ReAct`
- 先拆步骤、再执行：`Plan-and-Solve`
- 先做一版、再打磨：`Reflection`
- 有分支、回环、兜底：`Graph Flow`
- 两个角色轮流接力：`CAMEL`
- 多专家共享群聊推进：`AutoGen / Conversation`
- 一个总控统一派工：`Supervisor`
- 高并发、强隔离、强治理：`AgentScope`

如果想更系统地判断，可以看下面这张表：

| 模式 | 什么时候优先考虑 | 典型场景 | 不太适合 |
| --- | --- | --- | --- |
| `ReAct` | 任务本身不长，但执行过程中经常缺信息，需要边查边做 | 天气查询后推荐景点、查库存后回答用户、查接口状态后给诊断建议 | 步骤很长、前后依赖强、需要先规划整体路线的问题 |
| `Plan-and-Solve` | 任务步骤清晰、前后依赖明显，适合先产出一份执行计划 | 复杂数学题、长链路分析、结构化报告生成、迁移清单执行 | 强实时、强工具驱动、需要边做边改路线的场景 |
| `Reflection` | 第一版通常能做出来，但质量还需要继续提升 | 代码优化、文案润色、翻译审校、算法改进、测试用例补强 | 低价值、低容错、要求秒回的简单任务 |
| `Graph Flow` | 流程已经出现显式分支、循环、降级、人工介入或状态回放需求 | 检索失败走兜底、审批流、异常补偿流、多阶段问答管线、可观测工作流 | 只有一两步的小任务，或者不需要显式治理路径的简单链路 |
| `CAMEL` | 只有两个职责清晰、边界鲜明的角色，需要按协议轮流接力 | 产品经理和工程师、交易员和程序员、分析师和执行者 | 角色很多、需要全局动态派工或共享大群聊的系统 |
| `Conversation / AutoGen` | 多个专家需要围绕同一份共享历史轮流发言、共同推进 | 群聊式方案评审、需求分析 -> 编码 -> Review 的共享会话、多专家讨论 | 强顺序、强治理、每一步都要中心化决策的流程 |
| `Supervisor` | 需要一个总控统一拆任务、派任务、收结果、决定下一步 | 写作 -> 翻译 -> 审校，任务分发，多 Worker 协同，服务台式派单 | 平等协作群聊，或角色之间需要自由交棒的场景 |
| `AgentScope` | 重点已经不是“怎么让 Agent 更聪明”，而是“怎么让系统可治理、可隔离、可扩展” | 多租户 Agent 平台、消息驱动协作、高并发多会话、严格控制“谁能看到什么”的系统 | 低复杂度的单会话 Demo 或轻量串行流程 |

再换一个更贴近工作流选型的角度：

- 如果你关心的是“当前一步缺什么信息”，优先看 `ReAct`
- 如果你关心的是“整件事应该先拆成哪几步”，优先看 `Plan-and-Solve`
- 如果你关心的是“第一版做完以后怎样再提一轮质量”，优先看 `Reflection`
- 如果你关心的是“流程路径如何显式治理和回放”，优先看 `Graph Flow`
- 如果你关心的是“多个角色怎么协作”，再进入 `CAMEL / Conversation / Supervisor`
- 如果你关心的是“生产级上下文隔离、消息治理和并发控制”，再看 `AgentScope`

一句话总结这几个模式之间的关系：

- `ReAct / Plan-and-Solve / Reflection` 更像单 Agent 的不同增强路线
- `Graph Flow` 更像复杂工作流的状态机底座
- `CAMEL / Conversation / Supervisor` 更像多智能体协作策略
- `AgentScope` 更像生产级工程治理方向
