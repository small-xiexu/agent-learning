...
/**
 * OpenAI CAMEL Demo 配置支持类
 *
 * 职责：为真实 Demo 提供基于本地配置文件的 llm.api-key 检查，避免回退到环境变量或系统属性
 *
 * @author xiexu
 */
public final class OpenAiCamelDemoPropertySupport {

    private static final String API_KEY_PROPERTY = "llm.api-key";
    private static final String DEMO_ENABLED_PROPERTY = "demo.camel.openai.enabled";
    private static final String DEFAULT_MAIN_CONFIG = "application-openai-camel-demo.yml";
    private static final String DEFAULT_LOCAL_CONFIG = "application-openai-camel-demo-local.yml";
    private static final String EXAMPLE_API_KEY = "your-openai-api-key";

    private OpenAiCamelDemoPropertySupport() {
    }
    ...
    public static boolean hasConfiguredApiKey() {
        try {
            ...
            throw new IllegalStateException("加载 OpenAI CAMEL Demo 配置失败", exception);
        }
    }
