...
可以把框架版理解成一个“中心调度 + 三段流水线”：

```text
writer_agent -> translator_agent -> reviewer_agent -> FINISH
```

其中 `supervisor_router_agent` 不写任何业务内容，只看共享状态里的三个输出槽位：

```text
writer_output     为空 -> 返回 ["writer_agent"]
writer_output     非空且 translator_output 为空 -> 返回 ["translator_agent"]
translator_output 非空且 reviewer_output 为空 -> 返回 ["reviewer_agent"]
reviewer_output   非空 -> 返回 ["FINISH"]
```

这几行的真实含义不是“返回最终答案”，而是“告诉 `SupervisorAgent` 下一步该调度谁”。
初始状态下三个输出都为空，所以第一轮一定先调用 `writer_agent`；后面谁还是空，就说明哪一步还没完成。
