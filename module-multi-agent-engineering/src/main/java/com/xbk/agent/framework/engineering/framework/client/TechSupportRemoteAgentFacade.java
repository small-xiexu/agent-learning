...
public class TechSupportRemoteAgentFacade {
    ...
    /**
     * 远端 Agent 定位器。
     * 通过 Nacos 服务发现把逻辑服务名（如 "tech-support-agent"）解析为可访问的 HTTP URL。
    ...
    /**
     * A2A 公共配置。
     * 提供 Agent 服务命名（{@code config.getAgent().getTechName()}）和超时等参数。
     ...
     *   3. A2aHttpTransportSupport.sendMessage(url, params) → 发送给 Provider 的 POST / 端点
    ...
    public String call(String conversationId, String userRequest) {
        // 1. 从配置中取得技术专家 Agent 在 Nacos 注册的逻辑服务名（如 "tech-support-agent"）
        ...
        try {
            ...
            throw new RuntimeException("A2A 调用技术专家 Agent 失败: " + ex.getMessage(), ex);
        }
    }
